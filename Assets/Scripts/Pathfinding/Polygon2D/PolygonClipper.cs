...
namespace Polygon2D
{
    public class PolygonClipper
    {
        ...
        public static Polygon Compute(ref Polygon sp, ref Polygon cp, BoolOpType op)
        {
            ...
            Debug.Log("sp.TotalVertexCount = "+ sp.TotalVertexCount+ ", cp.TotalVertexCount = "+ cp.TotalVertexCount);
            HeapPriorityQueue<SweepEvent> eventQueue = new HeapPriorityQueue<SweepEvent>((sp.TotalVertexCount + cp.TotalVertexCount) * 2);
        ...
        private static void InsertPolygon(HeapPriorityQueue<SweepEvent> eventQueue, Polygon p, PolygonType pType)
        {
            ...
            {
                v1 = c.verticies[0];
                for (int iVert = 1; iVert < c.VertexCount; iVert ++)
                {
                ...
                se1.other = se2;
                se2.other = se1;
                Debug.Log("["+pType+"]Enqueue count = "+eventQueue.Count);
                eventQueue.Enqueue(se1);
                eventQueue.Enqueue(se2);
            }
        }
