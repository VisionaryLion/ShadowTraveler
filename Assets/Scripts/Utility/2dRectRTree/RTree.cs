...
namespace RTree
{
    ...
    public class RTree<T> where T : IEquatable<T>
    {
        ...
        private Stack<T> parents = new Stack<T>();
        //private TIntStack parentsEntry = new TIntStack();
        private Stack<T> parentsEntry = new Stack<T>();
        ...
        List<T> nearestIds = new List<T>();

        //the recursion methods require a delegate to retrieve data
        private delegate void processT(T item);
        ...
        private void init()
        {
            ...
            if (maxNodeEntries < 2)
            {
                Debug.LogWarning("Invalid MaxNodeEntries = " + maxNodeEntries + " Resetting to default value of " + DEFAULT_MAX_NODE_ENTRIES);
            ...
            {
                Debug.LogWarning("MinNodeEntries must be between 1 and MaxNodeEntries / 2");
            ...
            Debug.Log("init() " + " MaxNodeEntries = " + maxNodeEntries + ", MinNodeEntries = " + minNodeEntries);
        }
        ...
        public void Add(Rect r, T item)
        {
#if DEBUG
            Debug.Log("Adding Rect " + r + ", item " + item);
#endif
        ...
        private void add(Rect r, T item, int level)
        {
            ...
            {
                n.addEntryNoCopy(r, item);
            }
            else
            {
                newLeaf = splitNode(n, r, item);
            }
        ...
        /// <param name="item"></param>
        /// <returns></returns>
        private bool Delete(Rect r, T item)
        {
            ...
            while (foundIndex == -1 && parents.Count > 0)
            {
                ...
                if (!n.isLeaf())
                {
                    Debug.Log("searching Node<T> " + n.nodeId + ", from index " + startIndex);
                    bool contains = false;
        ...
        private void nearest(Point p, processT v, float furthestDistance)
        {
        ...
        /// <param name="r"></param>
        /// <returns></returns>
        public List<T> Intersects(Rect r)
        {
            List<T> retval = new List<T>();
            intersects(r, delegate (T item)
            {
                retval.Add(item);
            });
            return retval;
        ...
        private void intersects(Rect r, processT v)
        {
            Node<T> rootNode = getNode(rootNodeId);
        ...
        private void contains(Rect r, processT v)
        {
            // find all rectangles in the tree that are contained by the passed rectangle
        ...
        */
        ...
        private Node<T> splitNode(Node<T> n, Rect newRect, T newItem)
        {
...
#if DEBUG
            {
                Rect union = n.mbr.union(newRect);
            ...
            pickSeeds(n, newRect, newItem, newNode); // this also sets the entryCount to 1
            ...
            if (INTERNAL_CONSISTENCY_CHECKING)
            {
                ...
                {
                    Debug.Error("Error: splitNode old Node<T> MBR wrong");
                }

                if (!newNode.mbr.Equals(calculateMBR(newNode)))
                {
                    Debug.Error("Error: splitNode new Node<T> MBR wrong");
                }
            }
...
#if DEBUG
            {
                ...
                Debug.Debug("Node " + n.nodeId + " split. New area increased by " + percentageIncrease + "%");
            }
        ...
        private void pickSeeds(Node<T> n, Rect newRect, int newId, Node<T> newNode)
        {
...
#if DEBUG
            {
                Debug.Debug("pickSeeds(): NodeId = " + n.nodeId + ", newRect = " + newRect);
            }

            for (int d = 0; d < Rect.DIMENSIONS; d++)
            {
                ...
                for (int i = 0; i < n.entryCount; i++)
                {
                    ...
                    {
                        Debug.Error("Invalid normalized separation");
                    }

#if DEBUG
                    {
                        Debug.Debug("Entry " + i + ", dimension " + d + ": HighestLow = " + tempHighestLow +
        ...
        private int pickNext(Node<T> n, Node<T> newNode)
        {
            ...
            for (int i = 0; i < maxNodeEntries; i++)
            {
                if (entryStatus[i] == ENTRY_STATUS_UNASSIGNED)
                {
                    ...
                    if (n.entries[i] == null)
                    {
                        Debug.Error("Error: Node<T> " + n.nodeId + ", entry " + i + " is null");
                    }
...
#if DEBUG
                    {
                        Debug.Debug("Entry " + i + " group0 increase = " + nIncrease + ", group1 increase = " + newNodeIncrease +
        ...
        /// <param name="v"></param>
        /// <param name="n"></param>
        private void intersects(Rect r, processT v, Node<T> n)
        {
            for (int i = 0; i < n.entryCount; i++)
            {
                if (r.intersects(n.entries[i]))
        ...
        private Rect oldRectangle = new Rect(0, 0, 0, 0, 0, 0);
         ...
         */
        private Node<T> chooseNode(Rect r, int level)
        {
            ...
            while (true)
            {
                if (n == null)
                {
                    Debug.Error("Could not get root Node<T> (" + rootNodeId + ")");
                }
                ...
                {
                    Rect tempRectangle = n.getEntry(i);
        ...
        private Node<T> adjustTree(Node<T> n, Node<T> nn)
        {
            ...
            while (n.level != treeHeight)
            {
                ...
                {
                    Debug.Error("Error: entry " + entry + " in Node<T> " +
         ...
         */
        private void checkConsistency(int nodeId, int expectedLevel, Rect expectedMBR)
        {
            ...
            if (n == null)
            {
                Debug.Error("Error: Could not read Node<T> " + nodeId);
            }

            if (n.level != expectedLevel)
            {
                Debug.Error("Error: Node<T> " + nodeId + ", expected level " + expectedLevel + ", actual level " + n.level);
            }

            Rect calculatedMBR = calculateMBR(n);

            if (!n.mbr.Equals(calculatedMBR))
            {
                Debug.Error("Error: Node<T> " + nodeId + ", calculated MBR does not equal stored MBR");
            }

            if (expectedMBR != null && !n.mbr.Equals(expectedMBR))
            {
                Debug.Error("Error: Node<T> " + nodeId + ", expected MBR (from parent) does not equal stored MBR");
            }
            ...
            {
                Debug.Error("Error: Node<T> " + nodeId + " MBR using same rectangle object as parent's entry");
            }
            ...
            for (int i = 0; i < n.entryCount; i++)
            {
                if (n.entries[i] == null)
                {
                    Debug.Error("Error: Node<T> " + nodeId + ", Entry " + i + " is null");
                }
         ...
         */
        private Rect calculateMBR(Node<T> n)
        {
            Rect mbr = new Rect(n.entries[0].min, n.entries[0].max);
