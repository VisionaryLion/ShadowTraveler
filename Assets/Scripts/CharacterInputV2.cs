...
public class CharacterInputV2 : MonoBehaviour
{
    enum MovementState
    {
        WallSlide,
    ...
    //state defining vars
    MovementState _cState = MovementState.HMove;
    ...
    //external forces
    List<OutFadingForce> addOutFadingForce;

    //Trigger managment
    int _ladderTriggerCount = 0;
    int _frictionTriggerCount = 0;
    ...
    void Awake()
    {
        ...
        addOutFadingForce = new List<OutFadingForce>(2);
    }
    ...
    private void _motor_onTriggerExitEvent(Collider2D obj)
    {
        ...
        else if (obj.CompareTag(frictionTag))
        {
            _frictionTriggerCount--;
            if (_frictionTriggerCount == 0)
                _withFriction = false;
        }
    }
    ...
    void ApplyOutFadingForces()
    {
        OutFadingForce oFF;
        for (int i = 0; i < addOutFadingForce.Count; i++)
        {
            oFF = addOutFadingForce[i];
            _deltaExternalForces += oFF.force;
            oFF.DampForce(_motor.isGrounded);
            if (oFF.IsForceNull())
                addOutFadingForce.RemoveAt(i);
        }
    }
